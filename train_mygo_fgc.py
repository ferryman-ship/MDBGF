...
from module import MultiGenerator
...
def hybrid_negative_ce_loss(score, label, num_neg=20, hard_ratio=0.5, smoothing=0.1):
    """
    混合负采样：部分随机 + 部分困难
    """
    batch_size, num_entities = score.shape #[2048, 15000]
    device = score.device
    num_hard = int(num_neg * hard_ratio)
    num_rand = num_neg - num_hard

    # 正样本得分
    pos_score = score[torch.arange(batch_size), label]
    # print("pos_score", pos_score.size()) #2048

    # mask正样本以防止被选为负样本
    masked_score = score.clone()
    masked_score[torch.arange(batch_size), label] = -1e9
    # print("masked_score", masked_score.size()) #2048,15000

    # 得分最高的前num_hard个
    if num_hard > 0:
        hard_score, hard_idx = torch.topk(masked_score, k=num_hard, dim=1)
    else:
        hard_score = torch.empty(batch_size, 0, device=device)
        hard_idx = torch.empty(batch_size, 0, dtype=torch.long, device=device)

    # 随机负样本
    if num_rand > 0:
        rand_idx = torch.randint(0, num_entities, (batch_size, num_rand), device=device)
        mask = (rand_idx == label.unsqueeze(1))
        while mask.any():
            rand_idx[mask] = torch.randint(0, num_entities, (mask.sum().item(),), device=device)
            mask = (rand_idx == label.unsqueeze(1))
        rand_score = score.gather(1, rand_idx)
    else:
        rand_score = torch.empty(batch_size, 0, device=device)

    neg_score = torch.cat([hard_score, rand_score], dim=1)
    # print("neg_score", neg_score.size()) #2048,20

    combined_score = torch.cat([pos_score.unsqueeze(1), neg_score], dim=1)

    target = torch.zeros(batch_size, dtype=torch.long, device=device)
    criterion = nn.CrossEntropyLoss(label_smoothing=smoothing)
    loss = criterion(combined_score, target)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', default=2048, type = int) # 2048 batch
    ...
    #print("text_token_index", text_token_index)
    ...
    # optimizer_g = torch.optim.Adam(adv_generator.parameters(), lr = args.lr, weight_decay = args.decay)

    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, args.step_size, T_mult = 2)

    # scheduler_g = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_g, args.step_size, T_mult = 2)
    ...
    for epoch in range(last_epoch + 1, args.num_epoch + 1):
        ...
        for batch, label in KG_Loader:
            p_drop = min(0.3, epoch / args.num_epoch * 0.3)
            ent_embs, rel_embs = model(p_drop, gen = 0)
            scores = model.score(ent_embs, rel_embs, batch.cuda())

            # gen_ent_embs, gen_rel_embs = model(p_drop, gen = 1)
            # gen_scores = model.score(gen_ent_embs, gen_rel_embs, batch.cuda())

            # Hybrid Negative Sampling
            # loss = hybrid_negative_ce_loss(scores, label.cuda(), num_neg=200, hard_ratio=min(0.4, epoch / args.num_epoch * 0.4), smoothing=args.smoothing)

            loss = loss_fn(scores, label.cuda())
            # loss_g = loss_fn(gen_scores, label.cuda())
            # loss += loss_g * args.mu

            if args.mu != 0:
                #loss += model.contrastive_loss_finegrained(ent_embs) * args.mu
                loss += hybrid_negative_ce_loss(scores, label.cuda(), num_neg=20, hard_ratio=min(0.8, epoch / args.num_epoch * 0.8), smoothing=args.smoothing) * args.mu
            ...
            # torch.nn.utils.clip_grad_norm_(adv_generator.parameters(), 0.1)
            optimizer.step()
            # optimizer_g.step()
        scheduler.step()
        # scheduler_g.step()
        logger.info(f"{epoch} \t {total_loss:.6f} \t {time.time() - start:.6f} s")
        logger.info(f"{epoch} \t {total_loss_g:.6f} \t {time.time() - start:.6f} s")
